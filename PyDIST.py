"""
============================================================================
:class:`Distances` -- Distance metrics
============================================================================
...
class Distances(object):
    """
    Python distance/similarity module. Currently, includes distances from Cha [1].
    ...
    ----------
    distances: dict
        Dictionary containing distances for all the family members as outlined by Cha
    ...
    def __init__(self, P, Q):
        ...
        if len(P) != len(Q):
            raise ValueError("Arrays need to be of equal sizes...")
    ...
    def shannon(self):
        ...
        Q = self.Q
        return {
            'Kull-Leiber': sum(P * np.log(P / Q)),
            'Jeffreys': sum((P - Q) * np.log(P / Q)),
            'Kdivergence': sum(P * np.log(2 * P / (P + Q))),
            'Topsoe': sum(P * np.log(2 * P / (P + Q)) + Q * np.log(2 * Q / (P + Q))),
            'Jensen-Shan': 0.5 * sum(P * np.log(2 * P / (P + Q)) + Q * np.log(2 * Q / (P + Q))),
            'Jensen-Diff': 0.5 * sum(P * np.log(P) + Q * np.log(Q) - (P + Q) * np.log((P + Q) / 2.))
        }
    ...
    def combination(self):
        ...
        Q = self.Q
        return {
            'Taneja': 0.5 * sum((P + Q) * np.log((P + Q) / (2. * np.sqrt(P * Q))),
    ...
    def vicissitude(self):
        ...
        return {
            'Vicis-Wave Hedge': sum(abs(P - Q) / pqmin),
